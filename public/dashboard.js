...
async function init() {
  const basePath = location.pathname.replace(/\/$/, '');
  try {
    // Profile hydration + admin menu reveals are done centrally by topbar.js.
    // We just consume the resolved user for our own permission-gated setup.
    ...
    if (user?.supervisor) {
      // Only the dashboard needs the live-active card; not part of the shared
      // topbar hydration.
      const lc = document.getElementById('live-active-card');
      if (lc) lc.style.display = '';
    }

    // Preferences / User Permissions / Dashboard Settings / Announcement
    // Compose are wired by their own scripts (preferences-panel.js,
    // user-flags-panel.js, dashboard-settings-panel.js, announcement-compose.js)
    // which listen for rcs:user-loaded — no init calls needed here.

    // Announcement BANNER (the header banner showing active announcements)
    // is dashboard-only; the polling loop lives in initAnnouncements below.
    initAnnouncements();
...
function initAnnouncements() {
  // Only the dashboard shows the announcement banner. Compose modal wiring
  // lives in public/announcement-compose.js so it also works from the other
  // pages that now include the compose modal partial.
  window.isSupervisor = isSupervisor;
  window.isSuperAdmin = isSuperAdmin;
  fetchAnnouncements();
  setInterval(fetchAnnouncements, 30000);
  // Refetch when a new announcement gets posted from any page.
